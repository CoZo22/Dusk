// ============================================================
// DUSK APP — Full Supabase Integration
// SETUP:
...
html{font-size:clamp(14px,2.5vw,18px);-webkit-text-size-adjust:100%;text-size-adjust:100%}
...
.inp:focus{border-color:var(--violet);box-shadow:0 0 0 3px rgba(124,58,237,.12)}.inp::placeholder{color:var(--text3)}.inp[type="date"]{color-scheme:dark}
...
.topbar{height:calc(var(--top) + env(safe-area-inset-top));min-height:calc(var(--top) + env(safe-area-inset-top));background:rgba(10,7,22,.98);backdrop-filter:blur(20px);border-bottom:1px solid var(--border);display:flex;align-items:flex-end;justify-content:space-between;padding:0 18px calc(10px) ;padding-top:calc(12px + env(safe-area-inset-top));flex-shrink:0;position:relative;z-index:50}
...
// ─── SUPABASE UTILS ──────────────────────────────────────────
const addNotif = async (userId, message) => {
  await sb.from("notifications").insert({ user_id: userId, message });
};
...
function Ann({cu, profiles, toast}){
  ...
  useEffect(()=>{
    sb.from("posts").select("*, author:profiles(id,name,role,avatar_url)").order("created_at",{ascending:false}).then(({data})=>setPosts(data||[]));
    const ch=sb.channel("posts-ch").on("postgres_changes",{event:"*",schema:"public",table:"posts"},()=>{
      sb.from("posts").select("*, author:profiles(id,name,role,avatar_url)").order("created_at",{ascending:false}).then(({data})=>setPosts(data||[]));
    }).subscribe();
  ...
  const del=async(id)=>{await sb.from("posts").delete().eq("id",id);};
  const like=async(p)=>{
...
function Events({cu, profiles, toast}){
  ...
  useEffect(()=>{
    sb.from("events").select("*").order("created_at",{ascending:false}).then(({data})=>setEvents(data||[]));
    const ch=sb.channel("events-ch").on("postgres_changes",{event:"*",schema:"public",table:"events"},()=>{
      sb.from("events").select("*").order("created_at",{ascending:false}).then(({data})=>setEvents(data||[]));
    }).subscribe();
    return ()=>sb.removeChannel(ch);
  },[]);
      ...
      <div>
        ...
        {events.map(ev=>{
          ...
          return(<div key={ev.id} className="ev-card">
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}><button className={`ci-btn${ci?" ci-done":""}`} onClick={()=>!ci&&checkIn(ev)}>{ci?"✓ Checked In (+1pt)":"📡 GPS Check-In"}</button>{adm&&<button className="btn btn-d btn-sm" onClick={()=>del(ev.id)}>Delete</button>}</div>
          </div>);
        })}
      </div>
      <div className="lb-card">
        <div className="lb-head">🏆 Leaderboard</div>
        {sorted.slice(0,10).map((u,i)=>(<div key={u.id} className="lb-row"><div className={`lb-rank${i===0?" r1":i===1?" r2":i===2?" r3":""}`}>{i+1}</div><Av user={u} size="sm"/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{u.name}</div><div style={{fontSize:9,color:"var(--text3)",textTransform:"uppercase"}}>{u.role}</div></div><div className="pts">{u.points||0}pt</div></div>))}
      </div>
    </div>
  </div>);
}
...
function DM({cu, profiles, toast, initUser}){
  ...
  useEffect(()=>{
    if(!active)return;
    sb.from("direct_messages").select("*").or(`and(sender_id.eq.${cu.id},receiver_id.eq.${active.id}),and(sender_id.eq.${active.id},receiver_id.eq.${cu.id})`).order("created_at",{ascending:true}).then(({data})=>setMsgs(data||[]));
    const ch=sb.channel(`dm-${[cu.id,active.id].sort().join("-")}`)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"direct_messages"},payload=>{
        const m=payload.new;
        if((m.sender_id===cu.id&&m.receiver_id===active.id)||(m.sender_id===active.id&&m.receiver_id===cu.id))setMsgs(prev=>[...prev,m]);
      }).subscribe();
    return ()=>sb.removeChannel(ch);
  },[active?.id]);
  ...
  return(<div className="dm-wrap">
    <div className="dm-list">
      ...
      {members.map(u=>(<div key={u.id} className={`dm-li${active?.id===u.id?" on":""}`} onClick={()=>setActive(u)}><Av user={u} size="sm"/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600}}>{u.name}</div></div></div>))}
    </div>
    <div className="dm-chat">
      {!active?(<div className="empty" style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center"}}><div className="empty-ico">✉️</div><div>Select a member</div></div>):(
        <><div style={{padding:"9px 14px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",gap:10,background:"var(--bg2)",flexShrink:0}}><Av user={active} size="sm"/><div><div style={{fontSize:13,fontWeight:600}}>{active.name}</div><div style={{fontSize:10,color:"var(--text3)",textTransform:"uppercase",letterSpacing:1}}>{active.role}</div></div></div>
        <div className="chat-msgs">
          {msgs.length===0&&<div className="empty"><div className="empty-ico">💬</div><div>Start a conversation</div></div>}
          {msgs.map(m=>{const own=m.sender_id===cu.id;const u=own?cu:active;return(<div key={m.id} className={`cmsg${own?" own":""}`}><Av user={u} size="sm"/><div className={`minfo${own?" own":""}`}><div className="mbubble">{m.content}</div><div className="mtime">{fmt(m.created_at)}</div></div></div>);})}
          <div ref={ref}/>
        </div>
        <div className="chat-bar"><textarea className="cinput" value={msg} onChange={e=>setMsg(e.target.value)} placeholder={`Message ${active.name}...`} rows={1} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send()}}}/><button className="sendbtn" onClick={send}>➤</button></div></>
      )}
    </div>
  </div>);
}
  ...
  return(<div>
    <div style={{background:"var(--surface)",borderRadius:14,border:"1px solid var(--border)",overflow:"hidden",marginBottom:16}}>
      ...
      <div style={{padding:"0 18px 18px",marginTop:-50,position:"relative"}}>
        <div style={{display:"flex",alignItems:"flex-end",gap:12,marginBottom:12}}>
          ...
          {isOwn?(<label style={{cursor:"pointer",marginBottom:5}}><span className="btn btn-g btn-sm">Change Photo</span><input type="file" accept="image/*" onChange={upload} style={{display:"none"}}/></label>):(<button className="btn btn-p btn-sm" style={{marginBottom:5}} onClick={()=>onMsg(u)}>✉️ Message</button>)}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:5}}><h2 style={{fontFamily:"'Orbitron',sans-serif",fontSize:17,color:"var(--accent)"}}>{u.name}</h2><Rb role={u.role}/></div>
        {editing?(<div><textarea className="inp" value={bio} onChange={e=>setBio(e.target.value)} rows={3} style={{marginBottom:8}}/><div style={{display:"flex",gap:8}}><button className="btn btn-p btn-sm" onClick={saveBio}>Save</button><button className="btn btn-g btn-sm" onClick={()=>setEditing(false)}>Cancel</button></div></div>)
    ...
    <div className="g2">
      <div style={{background:"var(--surface)",borderRadius:13,border:"1px solid var(--border)",padding:15}}>
        ...
        {[["Points",<span className="pts">{u.points||0}pt</span>],["Events",evs.length],["Joined",new Date(u.created_at).toLocaleDateString()]].map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid rgba(45,38,80,.3)",fontSize:13}}><span style={{color:"var(--text2)"}}>{l}</span><span>{v}</span></div>))}
      </div>
      <div style={{background:"var(--surface)",borderRadius:13,border:"1px solid var(--border)",padding:15}}>
...
function Adm({cu, profiles, toast, onRefresh}){
  ...
  const setName=async(id)=>{if(!ne[id]?.trim())return;await sb.from("profiles").update({name:ne[id].trim()}).eq("id",id);await addNotif(id,`Your name: ${ne[id].trim()}`);setNe(n=>{...n,[id]:""});onRefresh();toast("Name set","success");};
  const genInv=async()=>{const c=gc();await sb.from("invite_codes").insert({code:c,created_by:cu.id});setLc(c);sb.from("invite_codes").select("*").order("created_at",{ascending:false}).then(({data})=>setCodes(data||[]));toast("Generated!","success");};
  const sendToReq=async(req)=>{const c=gc();await sb.from("invite_codes").insert({code:c,created_by:cu.id});await sb.from("invite_requests").update({status:"sent",code:c}).eq("id",req.id);setReqs(r=>r.map(x=>x.id===req.id?{...x,status:"sent",code:c}:x));toast(`Code: ${c}`,"success");};
  ...
  return(<div>
    <div className="tab-bar">{tabs.map(t=><button key={t.id} className="btn btn-g btn-sm" style={{...(tab===t.id?{background:"rgba(124,58,237,.15)",borderColor:"var(--violet)",color:"var(--accent)"})}} onClick={()=>setTab(t.id)}>{t.l}{t.c>0&&<span style={{marginLeft:4,background:"var(--violet)",color:"#fff",borderRadius:10,padding:"1px 6px",fontSize:9}}>{t.c}</span>}</button>)}</div>

    {tab==="members"&&<div style={{background:"var(--surface)",borderRadius:13,border:"1px solid var(--border)",overflow:"auto"}}><table className="atable"><thead><tr><th>Member</th><th>Role</th><th>Pts</th><th>Set Name</th><th></th></tr></thead><tbody>{profiles.filter(p=>p.approved).map(u=>(<tr key={u.id}><td><div style={{display:"flex",alignItems:"center",gap:8}}><Av user={u} size="sm"/>{u.name}</div></td><td>{u.id!==cu.id?<select className="inp" style={{padding:"4px 8px",width:"auto"}} value={u.role} onChange={e=>setRole(u.id,e.target.value)}><option value="member">Member</option><option value="moderator">Moderator</option><option value="admin">Admin</option></select>:<Rb role="admin"/>}</td><td><span className="pts">{u.points||0}pt</span></td><td><div style={{display:"flex",gap:5}}><input className="inp" style={{padding:"4px 8px",width:100,fontSize:12}} value={ne[u.id]||""} onChange={e=>setNe(n=>{...n,[u.id]:e.target.value})} placeholder="New name"/><button className="btn btn-g btn-sm" onClick={()=>setName(u.id)}>Set</button></div></td><td>{u.id!==cu.id&&<button className="btn btn-d btn-sm" onClick={()=>remove(u.id)}>✕</button>}</td></tr>))}</tbody></table></div>}

    {tab==="pending"&&<div>{pending.length===0&&<div className="empty"><div className="empty-ico">✓</div><div>No pending</div></div>}{pending.map(u=>(<div key={u.id} style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:12,padding:14,marginBottom:10,display:"flex",alignItems:"center",gap:11}}><Av user={u} size="md"/><div style={{flex:1}}><div style={{fontWeight:600}}>{u.name}</div><div style={{fontSize:12,color:"var(--text3)"}}>Code: {u.invite_code}</div></div><button className="btn btn-p btn-sm" onClick={()=>approve(u.id)}>Approve</button><button className="btn btn-d btn-sm" onClick={()=>remove(u.id)}>Reject</button></div>))}</div>}

    {tab==="codes"&&<div><div style={{marginBottom:12}}><button className="btn btn-p btn-sm" onClick={genInv}>Generate Code</button></div>{lc&&<div className="icode">New: {lc}</div>}<div style={{background:"var(--surface)",borderRadius:13,border:"1px solid var(--border)",overflow:"auto"}}><table className="atable"><thead><tr><th>Code</th><th>Status</th></tr></thead><tbody>{codes.map(c=>(<tr key={c.id}><td><span style={{fontFamily:"'JetBrains Mono'",fontSize:13,color:"var(--accent)"}}>{c.code}</span></td><td><span className={`rb ${c.used?"r-u":"r-a"}`}>{c.used?"Used":"Available"}</span></td></tr>))}</tbody></table></div></div>}

    {tab==="requests"&&<div>
      {reqs.length===0&&<div className="empty"><div className="empty-ico">📩</div><div>No requests</div></div>}
      {reqs.map(r=>(<div key={r.id} style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:12,padding:14,marginBottom:10}}>
        <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:10}}>
          {r.full_name&&<div style={{fontSize:13}}><span style={{color:"var(--text3)",fontSize:10,letterSpacing:2,textTransform:"uppercase",display:"block",marginBottom:2}}>Name</span><span style={{fontWeight:600}}>{r.full_name}</span></div>}
          <div style={{fontSize:13}}><span style={{color:"var(--text3)",fontSize:10,letterSpacing:2,textTransform:"uppercase",display:"block",marginBottom:2}}>Email</span><span>{r.email}</span></div>
          {r.phone&&<div style={{fontSize:13}}><span style={{color:"var(--text3)",fontSize:10,letterSpacing:2,textTransform:"uppercase",display:"block",marginBottom:2}}>Phone</span><span>{r.phone}</span></div>}
          {r.instagram&&<div style={{fontSize:13}}><span style={{color:"var(--text3)",fontSize:10,letterSpacing:2,textTransform:"uppercase",display:"block",marginBottom:2}}>Instagram</span><span style={{color:"var(--vlight)"}}>@{r.instagram.replace("@","")}</span></div>}
        </div>
        {r.message&&<div style={{fontSize:13,color:"var(--text2)",marginBottom:10,padding:"10px",background:"var(--surface2)",borderRadius:8,fontStyle:"italic"}}>"{r.message}"</div>}
...
export default function App() {
  ...
  const [af,setAf]=useState({email:"",pw:"",code:"",name:"",confirm:"",phone:"",instagram:"",why:"",dob:""});
  ...
  useEffect(()=>{
    ...
    if(navigator.permissions){
      navigator.permissions.query({name:'geolocation'}).then(r=>{
        if(r.state==='prompt') navigator.geolocation.getCurrentPosition(()=>{},()=>{});
      });
    }
  ...
  const register=async()=>{
    if(!af.dob)return toast("Date of birth required","error");
    if(!isAdult(af.dob))return toast("You must be 18 or older to join Dusk","error");
    ...
    toast("Registered! Confirm your email, then await admin approval.","success");
    setAm("login");
  };

  const reqInv=async()=>{if(!af.dob)return toast("Date of birth required","error");if(!isAdult(af.dob))return toast("You must be 18 or older to request an invitation","error");if(!af.email)return toast("Email required","error");if(!af.name)return toast("Full name required","error");if(!af.phone)return toast("Phone number required","error");if(!af.instagram)return toast("Instagram username required","error");await sb.from("invite_requests").insert({email:af.email,message:af.why,full_name:af.name,phone:af.phone,instagram:af.instagram});toast("Request submitted!","success");setAm("login");};
  ...
  if(!session||!cu)return(<><style>{S}</style>
    <div className="auth-wrap"><div className="auth-card">
      ...
      {am==="register"&&<><div className="stitle">Join Dusk</div>
        ...
        <AuthField label="Full Name (First & Last)" value={af.name} onChange={e=>afc("name",e.target.value)} placeholder="John Doe"/>
        <AuthField label="Date of Birth" type="date" value={af.dob} onChange={e=>afc("dob",e.target.value)} placeholder=""/>
        <AuthField label="Email" type="email" value={af.email} onChange={e=>afc("email",e.target.value)} placeholder="your@email.com"/>
      ...
      {am==="request"&&<><div className="stitle">Request Invitation</div>
        ...
        <AuthField label="Date of Birth" type="date" value={af.dob} onChange={e=>afc("dob",e.target.value)} placeholder=""/>
        <AuthField label="Email" type="email" value={af.email} onChange={e=>afc("email",e.target.value)} placeholder="your@email.com"/>
        <AuthField label="Phone Number" type="tel" value={af.phone} onChange={e=>afc("phone",e.target.value)} placeholder="+40 700 000 000"/>
        <AuthField label="Instagram Username" value={af.instagram} onChange={e=>afc("instagram",e.target.value)} placeholder="@username"/>
        <AuthField label="Why should you be chosen?" value={af.why} onChange={e=>afc("why",e.target.value)} placeholder="Tell us about yourself..."/>
