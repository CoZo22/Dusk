...
function Adm({cu, profiles, toast, onRefresh}){
  ...
  return(<div>
    ...
    {tab==="requests"&&<div>{reqs.length===0&&<div className="empty"><div className="empty-ico">📩</div><div>No requests</div></div>}{reqs.map(r=>(<div key={r.id} style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:12,padding:14,marginBottom:10}}><div style={{fontWeight:600,marginBottom:3}}>{r.email}</div>{r.message&&<div style={{fontSize:13,color:"var(--text2)",marginBottom:7}}>{r.message}</div>}{r.code&&<div className="icode">Code: {r.code}</div>}{r.status==="pending"&&<div style={{display:"flex",gap:7,marginTop:8}}><button className="btn btn-p btn-sm" onClick={()=>sendToReq(r)}>Send Code</button><button className="btn btn-g btn-sm" onClick={()=>dismissReq(r.id)}>Dismiss</button></div>}{r.status==="sent"&&<span className="rb r-m">Sent</span>}</div>))}</div>}
  </div>);
}
